...
![](https://raw.githubusercontent.com/leechungpa/DataScience-blog/master/content/posts/data/datamining.png)
...
![](https://raw.githubusercontent.com/leechungpa/DataScience-blog/master/content/posts/data/datamining-accuracy.png)
...
$R^2$는 분야별 유연한 기준 필요
    ...
    - 금융 경제 분야와 같이 관찰 데이터는 많은 변수와 인과관계 복잡 : $R^2$ 낮게 나옴
...
# plotting
plot(Price~Odometer,data=usedcar2,col=ifelse(usedcar2$Color=='silver','black',
                                             ifelse(usedcar2$Color=='white','blue','red')))
...
xyplot(charges~age|smoker,data=dat,panel=mypanel) # 조건부에서는 범주만 가능
xyplot(charges~sex|smoker,data=dat,panel=mypanel)
xyplot(charges~bmi|smoker,data=dat,panel=mypanel) # 흡연여부와 bmi의 교호작용을 의심할 수 있음
xyplot(charges~children|smoker,data=dat,panel=mypanel )
xyplot(charges~region|smoker,data=dat,panel=mypanel )

xyplot(charges~bmi|smoker*sex,data=dat,panel=mypanel ) #흡연여부와 남여부 총 4가지 조건부
xyplot(charges~age|smoker*sex,data=dat,panel=mypanel )
xyplot(charges~bmi|smoker*region,data=dat,panel=mypanel )

### Linear Regression ###
lm1=lm(charges ~ age+sex+bmi+children+smoker+region, data=dat) # 지시변수 안만들어도 자동으로 만들어 줌 
summary(lm1) 
plot(lm1,which=1) # 잔차분석 회귀진단 : 문제가 있음 (R^2은 높으나 문제 있음)
lm2=lm(charges ~ age+sex+bmi+children+smoker+region+bmi:smoker, data=dat) # 교호작용 포함
summary(lm2)
plot(lm2,which=1)
# identifying risk factors : 잔차의 패턴이 있는 이유를 세밀하게 찾기 위해 charges 말고 lm2$residuals 사용
xyplot(lm2$residuals~age|smoker,data=dat,panel=mypanel)
xyplot(lm2$residuals~sex|smoker,data=dat,panel=mypanel)
xyplot(lm2$residuals~bmi|smoker,data=dat,panel=mypanel) # 잔차에 패턴이 남아 있음
xyplot(lm2$residuals~children|smoker,data=dat,panel=mypanel)
xyplot(lm2$residuals~region|smoker,data=dat,panel=mypanel)
...
library(rpart.plot)
# maximal tree
tree1 = rpart(charges ~ age+sex+bmi+children+smoker+region, data = dat, method="anova")
      # rpart는 자동적으로 교호작용이 포함되므로 포함해서 작성하면 안됨
      # method="anova" : Y변수가 연속형이라는 의미

prp(tree1, type=4, extra=1, digits=3) # 시각화
      # 흡연자 안에 2그룹이 있는 것을 볼 수 있음 (age는 그래프상 크게 다르지 않으므로 사용X) : 흡연시 "bmi>=30" 찾음

### Categorizing "bmi>=30"
dat$risk <- dat$bmi >= 30
xyplot(charges~age|risk*smoker,data=dat,panel=mypanel)  # smoker*risk라고 해도 동일함
xyplot(charges~bmi|risk*smoker,data=dat,panel=mypanel)

lm3=lm(charges ~ age+sex+bmi+children+smoker+region+risk:smoker, data=dat)
      # bmi:smoker대신 risk:smoker로 바꿈
summary(lm3)
plot(lm3,which=1)
# identifying risk factors
xyplot(lm3$residuals~age|smoker,data=dat,panel=mypanel)
xyplot(lm3$residuals~sex|smoker,data=dat,panel=mypanel)
xyplot(lm3$residuals~bmi|smoker,data=dat,panel=mypanel)
xyplot(lm3$residuals~children|smoker,data=dat,panel=mypanel)
xyplot(lm3$residuals~region|smoker,data=dat,panel=mypanel)

lm4=lm(charges ~ age+sex+bmi+children+smoker+region+risk:smoker+bmi:smoker, data=dat)
        # lm3$residuals~bmi의 오차에 패턴이 있으므로 bmi:smoker도 추가
summary(lm4)
plot(lm4,which=1)

# identifying risk factors
xyplot(lm4$residuals~age|smoker,data=dat,panel=mypanel)
xyplot(lm4$residuals~sex|smoker,data=dat,panel=mypanel)
xyplot(lm4$residuals~bmi|smoker,data=dat,panel=mypanel) # 이전과 다르게 문제가 없음
xyplot(lm4$residuals~children|smoker,data=dat,panel=mypanel)
xyplot(lm4$residuals~region|smoker,data=dat,panel=mypanel)

step(lm4, direction = 'both') # lm4 is final model
...
# fitting full model
full_model = glm(RESPOND~. , family="binomial",data=directmail1)
summary(full_model)

# model's significance
empty_model = glm(RESPOND~1, family="binomial", data=directmail1)
...
plot(roccurve2, col="blue", add=TRUE,  print.auc=TRUE, print.auc.adj=c(0,-4))
legend("bottomright", legend=c("stepwise","simple"), col=c("red","blue"), lwd=2)
...
pred2 = prediction(prob_pred2, test$RESPOND)
...
nn1<-neuralnet(RESPOND~AGE+BUY18+CLIMATE+FICO+INCOME+MARRIED+OWNHOME+GENDER, 
               data=train, hidden=c(3,2), stepmax = 1e+05, threshold = 0.01, 
...
```{r}
# comparison
pred1<-compute(nn1,covariate=test[,-1]) #except "RESPOND"
...
4. M개의 모든 tree를 통해 계산한 OBB Error의 차이의 편균과 표준편차로 변수중요도 계산
    ...
    - i번째 변수의 변수 중요도 : $v_i={\bar{d}\over s_d}$
    
        - $\bar{d}=\sum_{j=1}^md_j/m$
        
        - $s_d^2=\sum_{j}^{m-1}(d_j-\bar{d})^2/(m-1)$
...
plot(roccurve, col="red", print.auc=TRUE, print.auc.adj=c(2.5,3), auc.polygon=TRUE)
plot(rocbag, col="green", print.auc=TRUE, print.auc.adj=c(2.5,1), add=T)
plot(rocboost, col="blue", print.auc=TRUE, print.auc.adj=c(2.5,-1), add=T)
...
4. 거리행렬 D중 새롭게 변화 되는 부분인 (UV) 와 다른 군집 W 사이의 거리 $d_{(UV)W}$를 계산
    ...
    - single linkage 단일연결법 : $d_{(UV)W}=\text{min}(d_{UW},d_{VW})$

    - complete linkage 완전연결법 : $d_{(UV)W}=\text{max}(d_{UW},d_{VW})$

    - average linkage 평균연결법 : $d_{(UV)W}={\sum^{n_{UV}}_{i=1}\sum^{n_{W}}_{i=1}d_{ij}\over n_{UV}n_W})$

    - centroid method 중심점연결법 : $d_{(UV)W}=\text{distance between the centroids of cluster UV and W}$
...
cent <- NULL
for (k in 1:3){
  cent <- rbind(cent, colMeans(tmp[tmp$cluster == k,]))
}
cent
x1 <- c(0,0,-6)
x2 <- c(0,0,0)
...
cent <- NULL
for (k in 0:2){
  cent <- rbind(cent, colMeans(USArrests[dbs$cluster == k,]))
}
cent
```
